...
describe('server responses', () => {
  ...
  it('should respond to a GET request for a swim command', (done) => {
    ...
    // router handles our GET req
    httpHandler.router(req, res);
    expect(res._responseCode).to.equal(200);
    expect(res._ended).to.equal(true);
    // OR || in Chai
    expect(res._data.toString()).to.satisfy((str) => {return (str === 'up' || str === 'down' || str === 'left' || str === 'right' || str === 'rest')});
  ...
  it('should respond with 404 to a GET request for a missing background image', (done) => {
    httpHandler.backgroundImageFile = path.join('.', 'spec', 'missing.jpg'); ///why use this instead of some garbage endpoint?
  ...
  var postTestFile = path.join('.', 'spec', 'water-lg.jpg'); //this file needs to be supplied for the post
